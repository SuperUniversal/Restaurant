...
# Python
python -m py_compile (Get-ChildItem -Path src -Filter *.py -Recurse).FullName
```
...
Get-ChildItem -Path "src" -Filter "*.ts" -Recurse | Select-String -Pattern "TODO"
...
1. Search for relevant terms:
   ```powershell
   Get-ChildItem -Filter "*.md" -Recurse | Select-String -Pattern "architecture"
   ```
